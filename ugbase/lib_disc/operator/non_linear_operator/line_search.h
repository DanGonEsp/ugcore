...
/// standard implementation of the line search based on the "sufficient descent"
template <typename TVector>
class TrustRegionMethod : public ILineSearch<TVector>
{
	public:
	//	type of
		typedef TVector vector_type;
	...
	///	default constructor (setting default values)
		TrustRegionMethod()
		 :	 m_maxSteps(10), m_lambdaStart(1.0), m_Delta(10000.0), m_lambdaReduce(0.5), m_alpha(0.25),
			 m_maxDefect(1e+10), m_verbose(true), m_bAcceptBest(false), m_bCheckAll(false), m_offset(""),
			 m_newtonUpdater(SPNULL)
//#if ENABLE_NESTED_NEWTON_RESOLFUNC_UPDATE
//			 ,
...
//#endif
	...
	///	constructor
		TrustRegionMethod(int maxSteps, number lambdaStart, number lambdaReduce, bool bAcceptBest)
		 :	 m_maxSteps(maxSteps), m_lambdaStart(lambdaStart), m_Delta(10000.0), m_lambdaReduce(lambdaReduce), m_alpha(0.25),
			 m_maxDefect(1e+10), m_verbose(true), m_bAcceptBest(bAcceptBest), m_bCheckAll(false), m_offset(""),
			 m_newtonUpdater(SPNULL)
//#if ENABLE_NESTED_NEWTON_RESOLFUNC_UPDATE
//			 ,
...
//#endif
	...
	///	constructor
		TrustRegionMethod(int maxSteps, number lambdaStart, number lambdaReduce, bool bAcceptBest, bool bCheckAll)
		 :	 m_maxSteps(maxSteps), m_lambdaStart(lambdaStart),m_Delta(10000.0), m_lambdaReduce(lambdaReduce), m_alpha(0.25),
			 m_maxDefect(1e+10), m_verbose(true), m_bAcceptBest(bAcceptBest), m_bCheckAll(bCheckAll), m_offset(""),
			 m_newtonUpdater(SPNULL)
//#if ENABLE_NESTED_NEWTON_RESOLFUNC_UPDATE
...
//#endif
	...
	///	returns information about configuration parameters
		...
		{
			std::stringstream ss;
			ss << "TrustRegionMethod( maxSteps = " << m_maxSteps << ", lambdaStart = " << m_lambdaStart << ", lambdaReduce = " << m_lambdaReduce << ", accept best = " <<
					(m_bAcceptBest ? "true" : "false") << " check all = " << (m_bCheckAll ? "true" : "false");
			return ss.str();
	...
	///	\copydoc ILineSearch::search
		virtual bool search(SmartPtr<IOperator<vector_type> > spOp,
							...
							vector_type& d, number defect)
		{
			PROFILE_BEGIN_GROUP(TrustRegionMethod_search, ""); // group?
		// 	clone pattern for s
			s.resize(u.size());
			p_aux.resize(p.size());
		//	start factor
